...
for trial in range(3):
    ...
    #################################### Train LS Network ####################################
    ##########################################################################################
    best_loss = 1e9; best_net = None; save_path = main_path + 'LabelSmoothing.bin'
    if os.path.isfile(save_path):
        print ("Model exists....Continue")
    else:
        print ("Model does not exist!....Training now:", title)
        for smoothing in smoothing_factors:
            criterion2 = LabelSmoothingCrossEntropy(smoothing = smoothing)
            title = "LabelSmoothing"; criterion = criterion2
            print('Train with: ', title)    
            net = get_new_model()
            net = net.to(device)
            ...
            if model.best_loss < best_loss:
                f = open(main_path + "LS_logs.txt", "w")
                f.write('Smoothing_Factor=' + str(smoothing))
        ...
        save_path = main_path + 'LSTmpScaling.bin'
